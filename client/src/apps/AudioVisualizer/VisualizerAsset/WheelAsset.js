...
export default class WheelAsset {
  ...
  addBars = () => {
    ...
    const { minBarHeight } = this;

    this.bars = [];

    this.radius = WIDTH > HEIGHT ? HEIGHT * 0.4 : WIDTH * 0.4;
    ...
    this.barWidth = ((2 * Math.PI * r) / sides) * 0.5;
    ...
    for (let i = 0; i < bcount; i++) {
      this.bouncers.push(
        Bodies.circle(centerX, centerY, Common.random(5, 18), {
          ...
          render: {
            ...appSS.ballStyle1,
            strokeStyle: getRgbaStr(rainbow[i], 0.15),
          },
        })
      );
    }
  ...
  update = ({ recordedMap, analyser, dataArray, isMute }) => {
    ...
    for (let i = 0; i < len; i++) {
      ...
      this.bars[i].render.fillStyle = getRgbaStr(rgb, 0.15);
    }
